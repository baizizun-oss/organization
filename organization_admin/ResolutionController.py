...
class listsHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        resolutions=common.select("organization", sql)
        print(resolutions)
        self.render("organization_admin\\templates\\Resolution\\lists.html",resolutions=resolutions)
...
class addHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        self.render("organization_admin\\templates\\Resolution\\add.html",users=users)


    def post(self):
        data={}
        data["content"] = self.get_argument("content")
        data["problem"] = self.get_argument("problem")
        data["executor"] = self.get_argument("executor")
        pass_time = self.get_argument("start_display_time_year")+","+self.get_argument("start_display_time_month")+","+self.get_argument("start_display_time_day")
        data["pass_time"] = str(int(time.mktime(time.strptime(pass_time,"%Y,%m,%d"))))
        sql = "insert into resolution(pass_time,content,problem,executor) values("+data["pass_time"]+","+data["content"]+","+data["problem"]+","+data["executor"]+")"
        ...
        if result:
            ...
            self.write('<html><head><title>提醒</title></head><body><script type="text/javascript">window.alert("出错了，请联系白老师！");</script></body></html>')
