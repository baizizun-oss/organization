...
class listsHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        self.render(os.path.join(common.BASE_DIR,"organization","templates","Competition","lists.html"),competitions=competitions,user_id=self.get_argument("user_id"))

class joinHandler(tornado.web.RequestHandler):
    def get(self):
        #统计
        common.tongji("competition_join")
        if self.get_cookie("user_id",None) ==None:#如果没有cookie就去登录
            ...
            #判断是否已经参赛
            sql="select * from competition_join where id ="+self.get_argument("id")+" and user_id ="+self.get_cookie("user_id")
            #print("sql:",sql)
            result=common.find("organization",sql)
            if result["id"]:
                self.write("<html><head><title>提醒</title></head><body><script type='text/javascript'>window.alert('你已报名不要重复报名！');window.location.href = '/organization/Competition/lists?user_id="+self.get_cookie("user_id")+"';</script></body></html>") 
            #判断当前用户是否可以报名（暂时主要是分数）
            sql="select * from user where id="+self.get_cookie("user_id")
            current_user=common.find("organization",sql)
            if current_user["points"]<competition["cost"]:
                self.write("<html><head><title>提醒</title></head><body><script type='text/javascript'>window.alert('你的积分不足！');window.location.href = '/organization/Competition/lists';</script></body></html>") 
            else:
                #扣分（包括增加用户扣分和增加分数变化表）并增加报名表
                sql="insert into competition_join(user_id,competition_id,enroll_time) values("+self.get_cookie("user_id")+","+self.get_argument("id")+","+str(int(time.time()))+");"
                sql=sql+"update user set points=points-"+str(competition["cost"])+";"
                sql=sql+"insert into user_score_history(user_id,ctime,score,score_source) values("+self.get_cookie("user_id")+","+str(int(time.time()))+","+str(competition["cost"])+",'competition');"
                result=common.execute("organization",sql)
                if result:
                    self.write("<html><head><title>提醒</title></head><body><script type='text/javascript'>window.alert('报名成功！请留意比赛时间，有问题可以联系白老师！');window.location.href = '/organization/Competition/lists?user_id="+self.get_cookie("user_id")+"';</script></body></html>") 
...
class detailHandler(tornado.web.RequestHandler):
    def get(self):
        sql="select * from competition where id="+self.get_argument("id")
        competition=common.find("organization",sql)
        self.render(os.path.join(common.BASE_DIR,"organization","templates","Competition","detail.html"),competition=competition)
...
class selectDifficultHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        self.render(os.path.join(common.BASE_DIR,"organization","templates","Question","select_Module.html"),difficult=self.get_argument("difficult"),module=self.get_argument("module"))
